...
class AboutEnumerations extends KoanSuite with ShouldMatchers {
     ...
     koan("You can extend the Enumeration by extending the Val class.") {
          ...
          Planets.Earth.mass should be(5.976E24) // (5.976e+24)
          Planets.Earth.radius should be(6378140.0) //(6378140.0)
     }


     koan("You can extend an Enumeration by extending the Val class and mixing in a trait"){

          trait Resilience

          //note here is how to make your enumerations: Bounciness and Flexibility, the subtypes
          // of trait Resilience.
          object Bounciness extends Enumeration {
               case class BounceResEnum() extends Val with Resilience
               type Bounciness = BounceResEnum
               //note important: when assigning to values, use () on case class name!
               val level1, level2, level3 = BounceResEnum()
          }

          object Flexibility extends Enumeration {
               case class FlexResEnum() extends Val with Resilience
               type Flexibility = FlexResEnum
               val type1, type2, type3 = FlexResEnum()
          }
          ...
          BouncingBall(level2).toString should be("BouncingBall(level2)")
          FlexingWall(type3).toString should be("FlexingWall(type3)")
     }
}
