...
class AboutHigherOrderFunctions extends KoanSuite {
     ...
     koan("function returning another function") {
          ...
          //addWithoutSyntaxSugar(2)(3).isInstanceOf[Function1[Int, Int]] should be (false)
     ...
     koan("function returning another function " +
          ...
          // @todo: ---- what does the part "= (y:Int)" mean? Does it return y? If so, how does it add it to x?
     ...
     koan(
          ...
          // @todo: ---- understand better what "x" means and how this is allowed to work given how function is declared
          // @todo: ---- understand what sideEffect is in this implementation
     ...
     koan("Currying is a technique to transform function with multiple parameters to function with one parameter") {
          ...
          //note the multiply _ means that we mean the function and haven't forgotten to provide args.
          // Different than in haskell, where we don't need _, just simply write 'multiply'
     ...
     koan("Control abstractions act like language keywords"){

          // note the => Boolean notation is call by name notation which
          //means () => Boolean
          // if we used call by value parameter-passing (without => ..)
          // then the parameters would be evaluated in the actual function call, which means
          // that the function would return false earlier... note help todo true or false?
          ...
          array should be(ArrayBuffer(9,8,7,6,5,4,3,2,1,0))
     }
}
