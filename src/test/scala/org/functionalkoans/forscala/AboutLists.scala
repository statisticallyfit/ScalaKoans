...
class AboutLists extends KoanSuite {
     ...
     koan("Lists are created easily multiple ways") {
          val a = List(1, 2, 3)
          a should be(List(1, 2, 3))

          val b = 1 :: 2 :: 3 :: Nil
          b should be(List(1, 2, 3))

          val c = List(1, 2.0, 33D, 4000L)
          c should be(List(1.0, 2.0, 33.0, 4000.0))

          val d = List[Number](1, 2.0, 33D, 4000L)
          d should be(List(1, 2.0, 33.0, 4000))

          val e = List.range(1, 10)
          e should be(List(1, 2, 3, 4, 5, 6, 7, 8, 9))

          val f = List.fill(3)("foo")
          f should be(List("foo", "foo", "foo"))

          val g = List.tabulate(5)(n => n * n)
          g should be(List(0, 1, 4, 9, 16))

          val h = "foo".toList
          h should be(List('f', 'o', 'o'))
     }
