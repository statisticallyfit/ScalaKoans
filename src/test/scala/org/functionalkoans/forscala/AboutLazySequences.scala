...
class AboutLazySequences extends KoanSuite {
     ...
     koan("Creating a lazy collection form a strict collection") {
          ...
          val lazyList = strictList.view      // @todo: ---- what does "view" do?
     ...
     /*koan("Strict collection always processes its elements but " +
       ...
       // @todo: ---- understand how the .map(f => f) piece works
       val strictList = List(inc _, inc _, inc _)
       strictList.map(f => f).head should be(1)
       x should be(3)
       strictList.map(f => f).head
       x should be(6)
       // @todo: ---- is this right: so here reapplying "map" leads from 1,2,3 to 4,5,6?
       ...
       x should be(2)
       // @todo: ---- why isn't the end list 4,5,6 like the first?
     }*/
     ...
     koan("Lazy collections could also be infinite") {
          ...
          Stream.continually(1).take(4).sum should be(4) // generates infinite list of 1,1,1,1,1...
     }
